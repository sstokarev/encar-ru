...
- `costItems` — строки расчёта, в том порядке, в каком их видит клиент. У строки есть `id`, `label` (текст для клиента) и `kind`:
  - `"kind": "fixed"` — сумма в рублях в поле `value` (брокер и СВХ);
  - `"kind": "krw"` — сумма в **вонах** в поле `value` (расходы по Корее: экспорт, фрахт). Она прибавляется к цене автомобиля **до** пересчёта в рубли, поэтому попадает в таможенную стоимость — так же, как это считает перевозчик. Рублёвой строкой то же самое посчиталось бы иначе;
  - `"kind": "percent"` — процент в поле `value`. Считается от таможенной стоимости, то есть от цены автомобиля **вместе** с корейскими расходами;
  - `"kind": "ladder"` — комиссия ступенями. Вместо `value` укажите `brackets`: массив вида `[{ "underRub": 1500000, "fee": 30000 }, …, { "fee": 100000 }]`. Ступень выбирается по сумме всех остальных строк, **до** комиссии. Граница `underRub` — строгая: при ровно 1 500 000 ₽ работает уже следующая ступень (50 000 ₽). Ступени идут по возрастанию, комиссия не может убывать, у последней ступени `underRub` не указывается. Такая строка может быть только одна;
  ...
  - `"kind": "formula", "value": "customs_v1"` — блок таможенных платежей: виджет сам разворачивает его в пошлину, утильсбор и сбор за оформление по таблицам `customs`. Такая строка должна быть **ровно одна** — без неё расчёт был бы уверенно занижен на миллионы, поэтому конфиг без неё виджет не примет.

  Сейчас в конфиге стоит реальная модель импортёра: расходы по Корее (воны), таможня, брокер (фикс), комиссия (ступени). Прочерков не осталось.

  Ещё два правила, о которые легко споткнуться: `id` строки не может быть `lot`, `duty`, `recycling`, `clearance` или `tariff-rounding` — эти имена виджет использует для строк, которые считает сам. И блок таможенных платежей округляется **вверх до 100 ₽** отдельной строкой «Округление тарифа» — так считает импортёр.
